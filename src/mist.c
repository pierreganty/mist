	...
	ISTSharingTree *frontier, *initial_marking;
{
	...
	struct timeval beforeIt, afterIt;
    float diff;

	times(&before);
	gettimeofday(&beforeIt, NULL);
		...
		if (ist_is_empty(frontier) == false) {
			...
			if (reached == true) {
				...
				if (file != NULL){
					...
					gettimeofday(&afterIt, NULL);
					diff = ((float)afterIt.tv_usec +(float)afterIt.tv_sec*1000000 - (float)beforeIt.tv_sec*1000000- (float)beforeIt.tv_usec);
...
static void print_help()
{
	...
	puts("     --plot=filename  generate filename.png with the graph of the memory usage");
}
	...
	ISTSharingTree *initial_marking, *bad, **lfp;
{
	...
	while (finished == false) {
		...
		if (file != NULL) fprintf(file, "0, \t 2.2, \t 0\n");
		fflush(NULL);
...
//			ist_write(bad);
			...
			if (file != NULL) fprintf(file, "0,\t 1.1,\t 0\n");
			fflush(NULL);
